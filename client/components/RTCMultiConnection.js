...
export default class RTCMultiConnection {
  constructor(roomid, forceOptions) {
    ...
    function SocketConnection(connection, connectCallback) {
      ...
      connection.socket.on('error', (event) => {
      ...
      connection.socket.on('set-isInitiator-true', (sessionid) => {
    ...
    function MultiPeers(connection) {
      ...
      const skipPeers = ['getAllParticipants', 'getLength', 'selectFirst', 'streams', 'send', 'forEach'];
      connection.peers = {
        getLength() {
          let numberOfPeers = 0;
          for (const peer in this) {
        ...
        send(data, remoteUserId) {
          ...
          if (remoteUserId) {
            const remoteUser = connection.peers[remoteUserId];
            if (remoteUser) {
              ...
              remoteUser.channels.forEach((channel) => {
                channel.send(data);
              });
              return;
            }
          ...
          this.getAllParticipants().forEach((participant) => {
            if (!that[participant].channels.length) {
              ...
              setTimeout(() => {
                that[participant].channels.forEach((channel) => {
                  channel.send(data);
                });
              }, 3000);
              return;
        ...
        return {
          ...
          onDataChannelMessage(message) {
            ...
            if (message instanceof ArrayBuffer || message instanceof DataView) {
              connection.fbr.convertToObject(message, (object) => {
            ...
            if (message.readyForNextChunk) {
              connection.fbr.getNextChunk(message, (nextChunk, isLastChunk) => {
                connection.peers[remoteUserId].channels.forEach((channel) => {
            ...
            connection.fbr.addChunk(message, (promptNextChunk) => {
      ...
      this.createNewPeer = function (remoteUserId, userPreferences) {
        ...
        const localConfig = this.getLocalConfig(null, remoteUserId, userPreferences);
      ...
      this.createAnsweringPeer = function (remoteSdp, remoteUserId, userPreferences) {
        ...
        const localConfig = this.getLocalConfig(remoteSdp, remoteUserId, userPreferences);
        connection.peers[remoteUserId] = new PeerInitiator(localConfig);
      };
      ...
      this.replaceTrack = function (track, remoteUserId, isVideoTrack) {
        ...
        const peer = connection.peers[remoteUserId].peer;
    ...
    (function () {
      ...
      if (isNodejs) {
        const version = process.versions.node.toString().replace('v', '');
      ...
      const regexIpv6 = /[a-f0-9]{1,4}(:[a-f0-9]{1,4}){7}/;
      ...
      function getIPs(callback, stream) {
        ...
        const pc = new RTCPeerConnection(servers, peerConfig);
        ...
        if (DetectRTC.isPromisesSupported) {
          ...
          pc.createOffer((result) => {
            pc.setLocalDescription(result, afterCreateOffer, () => {});
          }, () => {});
        }
      ...
      const isNodeWebkit = typeof process === 'object' && typeof process.versions === 'object' && process.versions['node-webkit'];
      ...
      if (!/^(https:|chrome-extension:)$/g.test(location.protocol || '')) {
        const isNonLocalHost = typeof document !== 'undefined' && typeof document.domain === 'string' && document.domain.search && document.domain.search(/localhost|127.0./g) === -1;
    ...
    function getRMCMediaElement(stream, callback, connection) {
      ...
      const mediaElement = document.createElement(isAudioOnly ? 'audio' : 'video');
      ...
      if (DetectRTC.browser.name === 'Firefox') {
        ...
        // fireEvent(stream, streamEndedEvent, stream);
          ...
          if (stream.type === 'local') {
            ...
            const streamEvent = connection.streamEvents[stream.streamid];
      ...
      const played = mediaElement.play();
    ...
    function isUnifiedPlanSuppored() {
      let isUnifiedPlanSupported = false;
    ...
    // if (DetectRTC.osName !== 'iOS') return;
      ...
      const iosrtc = cordova.plugins.iosrtc;
    ...
    function PeerInitiator(config) {
      ...
      const connection = config.rtcMultiConnection;
      ...
      peer.onicecandidate = function (event) {
        if (!event.candidate) {
          if (!connection.trickleIce) {
            const localSdp = peer.localDescription;
            config.onLocalSdp({
              type: localSdp.type,
              sdp: localSdp.sdp,
      ...
      peer.oniceconnectionstatechange = peer.onsignalingstatechange = function () {
        ...
        if (peer && peer.iceConnectionState && peer.iceConnectionState.search(/closed|failed/gi) !== -1 && self.streams instanceof Array) {
          self.streams.forEach((stream) => {
            const streamEvent = connection.streamEvents[stream.id] || {
              streamid: stream.id,
              stream,
              type: 'remote',
            };
      ...
      peer.ontrack = function (event) {
        ...
        const streamToShare = streamsToShare[event.stream.id];
        if (streamToShare) {
      ...
      this.createDataChannel = function () {
        const channel = peer.createDataChannel('sctp', {});
      ...
      this.enableDisableVideoEncoding = function (enable) {
        ...
        const parameters = rtcp.getParameters();
    ...
    var OnIceCandidateHandler = (function () {
      function processCandidates(connection, icePair) {
        const candidate = icePair.candidate;

        const iceRestrictions = connection.candidates;
        ...
        const protocol = connection.iceProtocols;
    ...
    window.currentUserMediaRequest = {
      ...
      remove(idInstance) {
        ...
        let stream = this.streams[idInstance];
        if (!stream) {
          return;
        }
        ...
        const options = stream.currentUserMediaRequestOptions;
    ...
    function getUserMediaHandler(options) {
      ...
      if (currentUserMediaRequest.streams[idInstance]) {
        ...
        const isBlackBerry = !!(/BB10|BlackBerry/i.test(navigator.userAgent || ''));
        ...
        if (typeof navigator.mediaDevices === 'undefined') {
          ...
          let getUserMediaStream; let
            getUserMediaError;
          navigator.mediaDevices = {
            getUserMedia(hints) {
              navigator.getUserMedia(hints, (getUserMediaSuccess) => {
        ...
        if (options.localMediaConstraints.isScreen === true) {
          if (navigator.mediaDevices.getDisplayMedia) {
            navigator.mediaDevices.getDisplayMedia(options.localMediaConstraints).then((stream) => {
              stream.streamid = stream.streamid || stream.id || getRandomString();
              stream.idInstance = idInstance;
            ...
            navigator.getDisplayMedia(options.localMediaConstraints).then((stream) => {
              stream.streamid = stream.streamid || stream.id || getRandomString();
              stream.idInstance = idInstance;
    ...
    const FileProgressBarHandler = (function () {
    ...
    const TranslationHandler = (function () {
      function handle(connection) {
        ...
        connection.Translator = {
          ...
          // if(location.protocol === 'https:') return callback(text);
            ...
            const sourceText = encodeURIComponent(text); // escape

            const randomNumber = `method${connection.token()}`;
            ...
            const source = `https://www.googleapis.com/language/translate/v2?key=${connection.googKey}&target=${connection.language || 'en-US'}&callback=window.${randomNumber}&q=${sourceText}`;
    ...
    (function (connection) {
      ...
      const mPeer = new MultiPeers(connection);

      const preventDuplicateOnStreamEvents = {};
      ...
      connection.openOrJoin = function (roomid, callback) {
        ...
        connection.checkPresence(roomid, (isRoomExist, roomid) => {
          if (isRoomExist) {
            ...
            const isOneWay = !!connection.session.oneway;
            const isDataOnly = isData(connection.session);
      ...
      connection.join = function (remoteUserId, options) {
        ...
        if ((remoteUserId && remoteUserId.session) || !remoteUserId || typeof remoteUserId === 'string') {
          const session = remoteUserId ? remoteUserId.session || connection.session : connection.session;
      ...
      connection.getUserMedia = connection.captureUserMedia = function (callback, sessionForced) {
        callback = callback || function () {};
        const session = sessionForced || connection.session;
      ...
      connection.onstream = function (e) {
        const parentNode = connection.videosContainer;
      ...
      connection.removeStream = function (streamid, remoteUserId) {
        ...
        connection.peers.getAllParticipants().forEach((participant) => {
          ...
          const user = connection.peers[participant];
          try {
          ...
          if (session.screen) {
            if (DetectRTC.browser.name === 'Edge') {
              navigator.getDisplayMedia({
                ...
                if ((session.audio || session.video) && !isAudioPlusTab(connection)) {
                  connection.invokeGetUserMedia(null, (stream) => {
                    gumCallback(stream);
                  });
                } else {
                  gumCallback(screen);
      ...
      connection.invokeGetUserMedia = function (localMediaConstraints, callback, session) {
        ...
        getUserMediaHandler({
          onGettingLocalMedia(stream) {
            const videoConstraints = localMediaConstraints.video;
            if (videoConstraints) {
      ...
      connection.resetTrack = function (remoteUsersIds, isVideoTrack) {
        ...
        remoteUsersIds.forEach((participant) => {
          const peer = connection.peers[participant].peer;
      ...
      connection.setStreamEndHandler = function (stream, isRemote) {
        ...
        stream.addEventListener(streamEndedEvent, () => {
          ...
          // reset remote "streams"
            const peer = connection.peers[streamEvent.userid].peer;
            var streams = [];
      ...
      connection.onmute = function (e) {
        ...
        if (e.muteType === 'both' || e.muteType === 'video') {
          ...
          const paused = e.mediaElement.pause();
      ...
      const skipStreams = ['selectFirst', 'selectAll', 'forEach'];
      ...
      connection.streamEvents = {
        ...
        selectAll(options) {
          ...
          const streams = [];
          Object.keys(connection.streamEvents).forEach((key) => {
            const event = connection.streamEvents[key];
      ...
      connection.checkPresence = function (roomid, callback) {
        ...
        if (SocketConnection.name === 'SSEConnection') {
          SSEConnection.checkPresence(roomid, (isRoomExist, _roomid, extra) => {
            if (!connection.socket) {
              if (!isRoomExist) {
        ...
        connection.socket.emit('check-presence', `${roomid}`, (isRoomExist, _roomid, extra) => {
          if (connection.enableLogs) {
      ...
      connection.getExtraData = function (remoteUserId, callback) {
        ...
        if (typeof callback === 'function') {
          connection.socket.emit('get-remote-user-extra-data', remoteUserId, (extra, remoteUserId, error) => {
